...
static uint64_t result_count = 1;
...
__global__ void sha256_kernel(char* out_input_string_nonce, unsigned char* out_found_hash, int *out_found, const char* in_input_string, size_t in_input_string_size, uint8_t difficulty, uint64_t nonce_offset) {
	...
	if (checkZeroPadding(sha, difficulty) && atomicExch(out_found, 1) == 0) {
		...
		memcpy(out_input_string_nonce+in_input_string_size, out, size+1);
	}
}
...
int main() {
	...
	std::cin >> result_count;
	while(result_count>0)
	{
		if(result_count<difficulty)
		{
			break;
		}
		else{
	...
	*g_found = 0;
	...
	//std::cout<<last_nonce_since_update<<std::endl;
	//std::cout<<"last_nonce_since_update"<<std::endl;
	pre_sha256();
	...
	int count=0;
	uint64_t tmp_difficulty = difficulty;
	while(count<result_count)
	{
		difficulty=tmp_difficulty-result_count+count+1;
		std::cout<<"Result_Difficulty_Number:"<<difficulty<<std::endl;
		while (!*g_found) {
